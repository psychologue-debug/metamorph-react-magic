  ...
  return (
    <div className={gridClass}>
      {mortals.map((mortal, i) => (
        <MortalToken key={mortal.id} mortal={mortal} compact={compact} size={size} index={i} />
      ))}
    </div>
  );
};

function MortalToken({ mortal, compact, size, index }: { mortal: Mortal; compact: boolean; size: string; index: number }) {
